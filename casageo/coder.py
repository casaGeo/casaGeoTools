...
code. Combining these two approaches is also supported.

If the input data combines street and house number information into a
single field, you can pass the resulting string to the ``street`` field
and leave the ``housenumber`` field unset.

If you specify a ``position`` for the search, the search will favor
places around that position and ``distance`` values will be computed for
the results.

The structured address fields are not hard filters, meaning it is
possible to receive results from outside the specified country or city.
In contrast, the ``countries`` list *is* a hard filter, limiting the
search to only the specified countries.
...
position : :class:`~shapely.Point`
    The geographical center of the search. Instead of a geometry object,
    you may also specify this as two separate fields
    ``position_latitude`` and ``position_longitude`` of type
    :class:`float`.

language : str
    The preferred language for the response. This must be a valid `IETF
    BCP47`_ language tag, such as ``"en-US"``, or a comma-separated list
    of such tags in order of preference.

political_view : str
    The political view of the query regarding disputed territories. This
    must be a valid `ISO 3166-1 alpha-3`_ country code.

limit : int
    Limit on the number of results to return for each query. Must be
    between 1 and 100. If unset, the default limit will be used.

countries : str | list[str]
    If set, the search is limited to the specified countries. Must be a
    sequence of valid `ISO 3166-1 alpha-3`_ country codes. This
    parameter treats a comma-separated string as a list of strings.

address_names_mode : str
    How to handle places with multiple names.

    - ``"default"`` -- Prefer matched names for administrative places
      and normalized names for street names.
...
postal_code_mode : str
    ...
    - ``"default"`` -- Return only one result per postal code, leaving
      the city or district name blank if necessary.
    - ``"cityLookup"`` -- When a postal code spans multiple cities,
      return all possible combinations of the postal code with the
      corresponding city names.
    - ``"districtLookup"`` -- When a postal code spans multiple
      districts, return all possible combinations of the postal code
      with the corresponding city and district names.
...
relevance : float
    The relevance of the result to the query as a number between zero
    and one.
...
columns below. Each of the values is a number between zero and one,
...
mq_street : float
    Average match quality of the ``streets`` field against each street
...
mq_housenumber : float
    Match quality of the ``housenumber`` field. If the requested house
    number could not be found, this indicates the numeric difference
    between the requested house number and the returned house number.
...
mq_ontologyname : float
    Match quality of the resulting ontology name against the input
    query.

Finally, the dataframe always contains the following error information
columns. These contain values only if the corresponding query could not
...
category_codes : bool
    Include HERE category, chain and food type identifiers in the
    result.
