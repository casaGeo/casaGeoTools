...
departure_info : bool
    Include additional information about the departure time and
    location.
...
position : :class:`~shapely.Point`
    The center point of the isolines. Instead of a geometry object, you
    may also specify this as two separate fields ``position_latitude``
    and ``position_longitude`` of type :class:`float`.

ranges : list[float]
    The distances or durations for which to calculate isolines.

ranges_unit : str
    The unit for the range values, either ``"minutes"`` or ``"meters"``.
    If unset, falls back to reading the field ``range_type`` which can
    either be ``"time"`` (minutes) or ``"distance"`` (meters).
...
arrival_time : datetime
    The date and time of arrival for time-dependent routing. This value
    is only used when ``direction`` is ``"incoming"``.
...
avoid_features : str | list[str]
    If set, these route features are avoided during routing. This
    parameter treats a comma-separated string as a list of strings.

exclude_countries : str | list[str]
    If set, these countries are excluded from routing. Must be a
...
departure_position : :class:`~shapely.Point`
    Resolved position of the departure location used for route
    calculation.
...
origin : :class:`~shapely.Point`
    The starting point of the route. Instead of a geometry object, you
    may also specify this as two separate fields ``origin_latitude`` and
    ``origin_longitude`` of type :class:`float`.

destination : :class:`~shapely.Point`
    The destination point of the route. Instead of a geometry object,
    you may also specify this as two separate fields
    ``destination_latitude`` and ``destination_longitude`` of type
    :class:`float`.
